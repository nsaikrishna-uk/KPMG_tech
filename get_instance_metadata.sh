#!/bin/bash
# Retrieve AWS instance metadata using curl.
# ./get_instance_metadata.sh help
# ./get_instance_metadata.sh id
...
roleProfile=$(curl -s http://169.254.169.254/latest/meta-data/iam/info \
    | grep -Eo 'instance-profile/([a-zA-Z0-9.-]+)' | sed 's#instance-profile/##')
